...
class App {
  ...
  constructor() {
    ...
    inputType.addEventListener('change', this._toogleElevationFields);
  ...
  _moveToPopup(e) {
    ...
    const clikedWorkout = this.#workouts.find(
      workout => workout.id === clickedWorkoutEl.dataset.id
    );

    this.#map.setView(clikedWorkout.coords, this.#mapZoomLevel, {
      animate: true,
      pan: {
        duration: 1,
      },
